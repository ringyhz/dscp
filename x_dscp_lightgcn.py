...
def parse_args():
    parser = argparse.ArgumentParser(description='Train and evaluate X-DSCP LightGCN.')
    parser.add_argument('--dataset', default='grocery', help='Dataset name supported by load_data.py.')
    parser.add_argument('--embedding-dim', type=int, default=32, help='Embedding dimension for the final model.')
    parser.add_argument(
        '--random-seeds',
        type=int,
        nargs='+',
    ...
    parser.add_argument('--eval-batch-size', type=int, default=1024, help='Batch size used during score prediction.')
    parser.add_argument(
        '--output',
        default=None,
        help='Optional output .npy file. Defaults to X-DSCP_LightGCN-<dataset>-results.npy next to the script.',
    )
...
class LightGCN(tf.keras.Model):
    def __init__(self, num_users, num_items, embedding_dim, adj_norm, n_layers=3):
        ...
        self.user_embeddings = tf.keras.layers.Embedding(num_users, embedding_dim, name='user_embeddings')
        self.item_embeddings = tf.keras.layers.Embedding(num_items, embedding_dim, name='item_embeddings')

    def propagate_embeddings(self):
        u_emb0 = self.user_embeddings(tf.range(self.num_users))
        i_emb0 = self.item_embeddings(tf.range(self.num_items))
...
class DCCLConf(tf.keras.Model):
    def __init__(self, user_embedding, item_embedding, adj_norm, n_layers, num_users, num_items, item_pop):
        ...
        self.item_embeddings = item_embedding
        self.adj_norm = adj_norm
        self.n_layers = n_layers
        self.num_users = num_users
        self.num_items = num_items
...
class AttentionCombiner(tf.keras.layers.Layer):
    def __init__(self):
        super().__init__()
        self.mlp = tf.keras.Sequential([tf.keras.layers.Dense(4, activation=None)])

    def call(self, e_per_u, e_pub_u, e_pop_i, e_nic_i, phi=None):
        if phi is None:
            phi = tf.zeros((tf.shape(e_per_u)[0], 0), dtype=e_per_u.dtype)
        features = tf.concat([e_per_u, e_pub_u, e_pop_i, e_nic_i, phi], axis=-1)
        logits = self.mlp(features)
        return tf.nn.softmax(logits, axis=-1), logits
...
class LightGCNCombineLayer(tf.keras.Model):
    ...
    def propa_emb(self, user_ids, item_ids):
        ...
        pos_item_latent1 = tf.gather(i_emb_1, item_ids)
        user_latent2 = tf.gather(u_emb_2, user_ids)
        pos_item_latent2 = tf.gather(i_emb_2, item_ids)
        return user_latent1, user_latent2, pos_item_latent1, pos_item_latent2

    def channel_scores(self, user_ids, item_ids):
        e_per_u, e_pub_u, e_pop_i, e_nic_i = self.propa_emb(user_ids, item_ids)
        scores = tf.concat(
            [
                dot(e_per_u, e_pop_i),
                dot(e_pub_u, e_pop_i),
        ...
        return scores, e_per_u, e_pub_u, e_pop_i, e_nic_i

    def score(self, user_ids, item_ids, phi=None):
        channel_score, e_per_u, e_pub_u, e_pop_i, e_nic_i = self.channel_scores(user_ids, item_ids)
        weights, logits = self.att(e_per_u, e_pub_u, e_pop_i, e_nic_i, phi)
        weighted_score = tf.reduce_sum(weights * channel_score, axis=-1, keepdims=True)
        fused_score = dot(tf.concat([e_per_u, e_pub_u], axis=-1), tf.concat([e_pop_i, e_nic_i], axis=-1))
        return weighted_score + fused_score, weights, channel_score, logits

    def call(self, user_ids, pos_item_ids, neg_item_ids, phi_pos=None, phi_neg=None):
        pos_score, _, _, _ = self.score(user_ids, pos_item_ids, phi_pos)
        neg_score, _, _, _ = self.score(user_ids, neg_item_ids, phi_neg)
...
def detect_normal_items(num_users, num_items, user_item_matrix, adj_norm, learning_rate, batch_size, embedding_dim, layers):
    ...
    user_embeddings = lightgcn_model.user_embeddings.get_weights()[0]
    item_embeddings = lightgcn_model.item_embeddings.get_weights()[0]
    predicted_scores = np.dot(user_embeddings, item_embeddings.T)
...
def evaluate_run(model, data, eval_batch_size):
    num_users = data['num_users']
    num_items = data['num_items']
    test_a_mat = data['test_a1']
    test_a_mat_div = data['test_a1_div']

    predicted_scores = evaluation.predict_interaction_matrix(
        model,
        num_users,
        num_items,
...
def run_experiment(dataset, random_seed, embedding_dim, alpha, beta, layers, epochs, batch_size, learning_rate, eval_batch_size):
    ...
    return evaluate_run(model, data, eval_batch_size=eval_batch_size)
